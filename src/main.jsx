...
const Icon = ({ children, size = 20 }) => (
  <span className="ico" style={{ width: size, height: size, fontSize: Math.max(12, size * 0.72) }}>{children}</span>
);

const nav = [
  ['COMMAND', '⌂'], ['CORE', '◉'], ['SUPERVISOR', '✺'], ['TIME MACHINE', '◷'], ['HORIZON', '⌁'], ['INTELLIGENCE', '✦'], ['DNA', '⟐'], ['OPPORTUNITIES', '◎'], ['WEATHER', '☼'], ['REPORTS', '▤'], ['SETTINGS', '⚙']
];
const signals = [
  ['BUY', '+12', 'cyan'], ['GAIN', '+8', 'cyan'], ['FLOW', '+15', 'cyan'], ['CENTRE', '+7', 'cyan'], ['LOCK', '+9', 'green'], ['DRIFT', '-6', 'amber'], ['SELL', '-8', 'pink'], ['LOSS', '-4', 'pink'], ['BLOCK', '-2', 'amber']
];
const dims = [
  ['TRUST', 82, 'STRONG', '⛨'], ['CUSTOMERS', 76, 'GOOD', '●●'], ['PRODUCT', 88, 'EXCELLENT', '⬡'], ['MARKET POSITION', 67, 'FAIR', '◎'], ['GROWTH', 69, 'IMPROVING', '↗'], ['LEADERSHIP', 84, 'STRONG', '♛'], ['FINANCIAL HEALTH', 77, 'GOOD', '▥']
];
...
function Sidebar(){return <aside className="sidebar"><div className="brand"><div className="logo"><Icon size={25}>◇</Icon></div><div><b>ReCentre OS</b><span>EXECUTIVE COMMAND</span></div></div><nav>{nav.map(([n,g],i)=><button className={i===0?'active':''} key={n}><Icon size={20}>{g}</Icon><span>{n}</span>{i>0&&<Icon size={14}>›</Icon>}</button>)}</nav><div className="statusOrb"><div className="miniCore"><span/></div><p>SYSTEM STATUS</p><b>OPTIMAL</b></div></aside>}
function Topbar(){return <header className="topbar"><div className="pill dark"><Icon size={17}>◌</Icon> PELOTON INTERACTIVE</div><div className="pill level">LEVEL 7</div><div className="muted">RECOVERY IN PROGRESS</div><div className="progress"><span>RECOVERY PROGRESS</span><div><i/></div><b>72%</b></div><button className="round">◔</button><div className="user"><span>PR</span><div><b>Paul</b><small>Executive</small></div></div></header>}
...
function CoreDimensions(){return <section className="dimensions"><h3>CORE DIMENSIONS</h3><div className="dimGrid">{dims.map(([n,v,l,g])=><Panel className="dim" key={n}><Icon size={32}>{g}</Icon><div><p>{n}</p><strong>{v}</strong><b>{l}</b></div><span>↑</span><Spark /></Panel>)}</div></section>}
